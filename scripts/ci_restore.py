...
test-results bucket, etc.). Unlike jupyter-deploy's ci_restore.py this does NOT hard-code
bot/oauth secret names — this repo has no GitHub apps, so we restore whatever masked
secrets the deploy has generically via `jd config --restore-secrets`.
...
def main() -> None:
    ...
    # --restore-secrets restores whatever secrets the deploy has (no hard-coded names).
    print("\nRestoring secrets and configuring...")
    run_jd_config(["--restore-secrets"], str(ci_dir))
